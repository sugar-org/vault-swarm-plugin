...
info "Enabling KV v2 engine at custom mount path '${VAULT_MOUNT_PATH}'..."
docker exec "${VAULT_CONTAINER}" \
    env VAULT_ADDR="${VAULT_ADDR}" VAULT_TOKEN="${VAULT_ROOT_TOKEN}" \
    vault secrets enable -version=2 -path="${VAULT_MOUNT_PATH}" kv
success "Enabled KV v2 engine at ${VAULT_MOUNT_PATH}/."
...
docker exec "${VAULT_CONTAINER}" \
    env VAULT_ADDR="${VAULT_ADDR}" VAULT_TOKEN="${VAULT_ROOT_TOKEN}" \
    vault kv put \
    "${VAULT_MOUNT_PATH}/${SECRET_PATH}" \
    "${SECRET_FIELD}=${SECRET_VALUE}"
success "Secret written: ${VAULT_MOUNT_PATH}/${SECRET_PATH} ${SECRET_FIELD}=${SECRET_VALUE}"
...
success "Vault smoke test PASSED with custom mount path '${VAULT_MOUNT_PATH}' (incl. rotation)"
